...
from crams_allocation_review.models import ReviewDate
from crams_allocation_review.models import ReviewConfig
...
def get_erb_notify_period(e_research_body):
    try:
        return config_init.eSYSTEM_REVIEW_NOTIFY_PERIOD_MAP[
            e_research_body.name.lower()]
    except:
...
def get_erb_review_period(e_research_body):
    try:
        return config_init.eSYSTEM_REVIEW_PERIOD_MAP[
            e_research_body.name.lower()]
    except:
        raise exceptions.ParseError(
