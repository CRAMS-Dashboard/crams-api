...
class SoftwareProduct(crams.ProvisionableItem):
    ...
    e_research_body = models.ForeignKey(crams.EResearchBody)

    category = models.ForeignKey(SoftwareProductCategory)
...
class SoftwareProductMetaData(models.Model):
    name = models.ForeignKey(crams.EResearchBodyIDKey)

    value = models.CharField(max_length=99)  # mysql innoDB size limit 767

    software = models.ForeignKey(SoftwareProduct, related_name='metadata')
...
class SoftwareLicense(models.Model):
    ...
    software = models.ForeignKey(SoftwareProduct, related_name='licenses')
    ...
    type = models.ForeignKey(SoftwareLicenseType)
...
class ContactSoftwareLicense(crams.ProvisionableItem):
    ...
    contact = models.ForeignKey(
        contact_models.Contact, related_name='software_licenses')

    status = models.CharField(max_length=1, choices=ACTION_CHOICES)

    license = models.ForeignKey(SoftwareLicense, related_name='user_licenses')
